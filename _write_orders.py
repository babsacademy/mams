...
<div>
    <div class="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            ...
            <p class="text-[10px] text-zinc-500 dark:text-zinc-400 uppercase tracking-widest font-bold mt-1">G''' + "\u00e9" + r'''rez vos commandes et leurs statuts en temps r''' + "\u00e9" + r'''el.</p>
        </div>
    </div>
    ...
    <div class="flex flex-col sm:flex-row gap-4 mb-8">
        <div class="flex-1 relative group">
            <flux:input wire:model.live.debounce.300ms="search" placeholder="Rechercher par nom, t''' + "\u00e9" + r'''l''' + "\u00e9" + r'''phone ou n''' + "\u00b0" + r''' de commande…" icon="magnifying-glass" variant="filled" class="!bg-white dark:!bg-zinc-900 border-zinc-200 dark:border-zinc-800 !h-11 font-bold" />
        </div>
                    ...
                    @forelse($this->orders as $order)
                        @php
                            $statusConfig = [
                                ...
                                'confirmed' => ['color' => 'blue',   'label' => 'Confirm''' + "\u00e9" + r'''e'],
                                'shipped'   => ['color' => 'purple', 'label' => 'Exp''' + "\u00e9" + r'''di''' + "\u00e9" + r'''e'],
                                'delivered' => ['color' => 'green',  'label' => 'Livr''' + "\u00e9" + r'''e'],
                                'cancelled' => ['color' => 'red',    'label' => 'Annul''' + "\u00e9" + r'''e'],
                            ];
                        ...
                        <tr class="group hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-all duration-300">
                            <td class="pl-10 pr-6 py-5 whitespace-nowrap">
                                <div class="flex flex-col">
                                    <span class="font-black font-mono text-zinc-900 dark:text-zinc-100 group-hover:text-brand-pink transition-colors">{{ $order->order_number ?: 'CMD-'.str_pad($order->id, 5, '0', STR_PAD_LEFT) }}</span>
                            ...
                            <td class="px-6 py-5 whitespace-nowrap">
                                <div class="flex items-center gap-3.5">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <a href="tel:{{ $order->customer_phone }}" class="text-xs text-zinc-400 hover:text-brand-pink transition-colors mt-0.5 font-medium block">{{ $order->customer_phone }}</a>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-5 whitespace-nowrap">
                                <div class="flex flex-col items-center gap-2">
                                    <span class="inline-block px-2.5 py-0.5 text-[9px] font-black uppercase tracking-wider rounded-lg border {{ $badgeClass }}">
                                        {{ $current['label'] }}
                                    </span>
                                    <flux:select wire:change="updateStatus({{ $order->id }}, $event.target.value)"
                                                 class="max-w-[140px] !h-8 !text-[10px] !font-bold !rounded-xl"
                                                 value="{{ $order->status }}">
                                        @foreach(App\Models\Order::STATUSES as $key => $label)
                            ...
                            <td class="pl-6 pr-10 py-5 text-right whitespace-nowrap">
                                <div class="flex items-center justify-end gap-1 sm:opacity-0 sm:group-hover:opacity-100 transition-all transform sm:translate-x-2 sm:group-hover:translate-x-0">
                                    <flux:button href="{{ route('admin.orders.show', $order) }}" size="sm" variant="ghost" icon="eye" inset class="text-zinc-400 hover:text-brand-pink" />
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-10 py-32 text-center">
                                ...
                                <h3 class="text-xl font-black text-zinc-900 dark:text-white uppercase tracking-tight">Aucune commande trouv''' + "\u00e9" + r'''e</h3>
                                <p class="text-sm text-zinc-400 mt-2 max-w-xs mx-auto">Les commandes appara''' + "\u00ee" + r'''tront ici d''' + "\u00e8" + r'''s qu'un client passera commande.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
...
filepath = os.path.join(r'c:\dev\schic\resources\views\components\admin\orders', '\u26a1index.blade.php')
