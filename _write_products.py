...
path = os.path.join('c:/dev/schic/resources/views/components/admin/products', '\u26a1index.blade.php')
content = r"""<?php
...
new #[Title('Produits')] #[Layout('layouts.app')] class extends Component
{
    ...
    public function toggleActive(int $productId): void
    {
        ...
        $this->dispatch('notify', message: 'Statut mis """ + "\u00e0" + r""" jour.');
    }
    ...
    public function deleteProduct(): void
    {
        if ($this->confirmDeleteId) {
            ...
            $this->dispatch('notify', message: 'Produit supprim""" + "\u00e9" + r""".');
        }
    }
...
<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
        <div>
            ...
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">G""" + "\u00e9" + r"""rez vos produits, stocks et visibilit""" + "\u00e9" + r""".</p>
        </div>

        <flux:button href="{{ route('admin.products.create') }}" variant="primary" class="!bg-brand-pink border-none font-black uppercase tracking-widest text-[10px] py-3 shadow-lg shadow-brand-pink/20">
    ...
    <div class="flex flex-col sm:flex-row gap-4 mb-8">
        <div class="flex-1 relative group">
            <flux:input wire:model.live.debounce.300ms="search" placeholder="Chercher une r""" + "\u00e9" + r"""f""" + "\u00e9" + r"""rence, un nom…" icon="magnifying-glass" variant="filled" class="!bg-white dark:!bg-zinc-900 border-zinc-200 dark:border-zinc-800 !h-11 font-bold" />
        </div>
        <flux:select wire:model.live="filterStatus" class="sm:w-64 !h-11">
            ...
            <flux:select.option value="inactive">Produits Masqu""" + "\u00e9" + r"""s</flux:select.option>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-zinc-200 dark:border-zinc-800/50">
                        ...
                        <th class="text-center px-6 py-5 text-xs font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-[0.15em] whitespace-nowrap">Visibilit""" + "\u00e9" + r"""</th>
                        ...
                        <tr class="group hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-all duration-300">
                            <td class="px-6 py-5 whitespace-nowrap max-w-xs">
                                <div class="flex items-center gap-4">
                                    <div class="relative size-14 rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 shrink-0">
                                        @if($product->image_url)
                                            <img src="{{ Str::startsWith($product->image_url, ['http://', 'https://']) ? $product->image_url : asset('storage/' . $product->image_url) }}" alt="{{ $product->name }}"
                                                 loading="lazy"
                                    ...
                                    <div class="min-w-0">
                                        <p class="font-black text-zinc-900 dark:text-zinc-100 leading-tight text-base group-hover:text-brand-pink transition-colors truncate">{{ $product->name }}</p>
                                        <div class="flex items-center gap-2 mt-1.5">
                                            ...
                                            @if($product->badge)
                                                <span class="px-2 py-0.5 bg-brand-pink/10 text-[9px] uppercase font-black text-brand-pink rounded tracking-wider border border-brand-pink/20">{{ $product->badge }}</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                            ...
                            <td class="px-6 py-5 hidden md:table-cell whitespace-nowrap">
                                <span class="inline-flex items-center px-3 py-1.5 bg-zinc-100 dark:bg-zinc-800/50 text-zinc-500 dark:text-zinc-400 text-[10px] font-black uppercase tracking-widest rounded-full border border-zinc-200/50 dark:border-zinc-700/50">
                                    {{ $product->category?->name ?? 'Ind""" + "\u00e9" + r"""fini' }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-5 text-center hidden sm:table-cell whitespace-nowrap">
                                ...
                                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 {{ $stockBg }} text-[10px] font-black uppercase tracking-widest rounded-2xl border">
                                    ...
                                    {{ $product->stock }} <span class="opacity-70">Unit""" + "\u00e9" + r"""s</span>
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-5 text-right whitespace-nowrap">
                                <div class="flex items-center justify-end gap-1 sm:opacity-0 sm:group-hover:opacity-100 transition-all sm:transform sm:translate-x-2 sm:group-hover:translate-x-0">
                                    <flux:button href="{{ route('admin.products.edit', $product) }}" size="sm" variant="ghost" icon="pencil-square" inset class="text-zinc-400 hover:text-brand-pink" />
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-32 text-center">
                                ...
                                <p class="text-sm text-zinc-400 mt-2 max-w-xs mx-auto">Votre inventaire est vide. Ajoutez vos premi""" + "\u00e8" + r"""res cr""" + "\u00e9" + r"""ations pour commencer """ + "\u00e0" + r""" vendre.</p>
                                <flux:button href="{{ route('admin.products.create') }}" variant="filled" class="mt-8 !bg-zinc-900 dark:!bg-white dark:!text-zinc-900 font-black uppercase tracking-widest text-[9px]">Cr""" + "\u00e9" + r"""er le premier produit</flux:button>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    @if($confirmDeleteId)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-6" x-data x-on:keydown.escape.window="$wire.set('confirmDeleteId', null)">
            ...
            <div class="relative bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl p-10 w-full max-w-md border border-zinc-200 dark:border-zinc-800 text-center animate-in zoom-in-95 duration-200">
                ...
                <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-3">Cette action est <span class="font-black text-rose-500 uppercase tracking-widest text-[10px]">irr""" + "\u00e9" + r"""versible</span>. Vous perdrez toutes les donn""" + "\u00e9" + r"""es li""" + "\u00e9" + r"""es """ + "\u00e0" + r""" ce produit.</p>
